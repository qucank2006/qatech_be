...
// Helper: Tạo orderCode duy nhất
function generateOrderCode() {
  return `ORD${Date.now()}${Math.floor(Math.random() * 1000)}`;
}

// Helper: Thêm vào statusHistory
...
exports.getOrderById = async (req, res) => {
  try {
    ...
    // Chỉ admin/employee hoặc chủ đơn hàng mới xem được
...
exports.getAllOrders = async (req, res) => {
  try {
    ...
    if (dateFrom || dateTo) {
      ...
      if (dateTo) {
        // Thêm 1 ngày để bao gồm cả ngày dateTo
...
// Admin/Employee: Cập nhật trạng thái đơn hàng (trừ hủy - chỉ khách hàng được hủy)
exports.updateOrderStatus = async (req, res) => {
  ...
  try {
    ...
    // Admin/Employee chỉ được cập nhật các trạng thái này (KHÔNG được hủy)
...
exports.cancelOrder = async (req, res) => {
  ...
  try {
    ...
    // Chỉ chủ đơn hàng mới hủy được
    if (order.userId !== userId) {
      await transaction.rollback();
      return res.status(403).json({ msg: "Bạn không có quyền hủy đơn hàng này" });
    }

    // Chỉ hủy được khi đơn hàng đang pending hoặc paid
